...
_M.parseRequest = function(u, b, type)
    local reqt = {
        url = u,
        target = {},
    }
...
_M.request = socket.protect(function(reqt, body, type)
    if base.type(reqt) == "string" then
        if type == nil then type = "x-www-form-urlencoded" end
        reqt = _M.parseRequest(reqt, body, type)
